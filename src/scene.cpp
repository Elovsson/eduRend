...
void OurTestScene::Render()
{
	...
	UpdateLightCameraBuffer(/*{ 0.0f, 20.0f, 20.0f, 0.0f }*/m_camera->GetCamPosition(), m_camera->GetCamPosition());
...
void OurTestScene::InitMaterialBuffer()
{
	...
	D3D11_BUFFER_DESC matrixBufferDesc = { 0 };
	matrixBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	matrixBufferDesc.ByteWidth = sizeof(Material);
	matrixBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	matrixBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	matrixBufferDesc.MiscFlags = 0;
	matrixBufferDesc.StructureByteStride = 0;
...
void OurTestScene::UpdateMaterialBuffer(float3 specular,
	float3 diffuse,
	float3 ambient,
	float shininess)
{
